...
@interface ParametricEqFilterVC ()
{
...
@property (nonatomic, strong) AEParametricEqFilter * peFilter;

@end

@implementation ParametricEqFilterVC
...
- (void)audioConfing
{
    ...
    _peFilter = [[AEParametricEqFilter alloc] init];
    _peFilter.centerFrequency = [_oneValueLabel.text floatValue];
    _peFilter.qFactor = [_twoValueLabel.text floatValue];
    _peFilter.gain = [_threeValueLabel.text floatValue];
}
...
- (IBAction)onPlay:(UIButton *)sender {
    ...
    [_audioController addFilter:_peFilter];
...
- (IBAction)onPause:(UIButton *)sender {
    ...
    [_audioController removeFilter:_peFilter];
}
