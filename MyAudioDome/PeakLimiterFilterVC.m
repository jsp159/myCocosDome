...
@property (nonatomic, strong) AEPeakLimiterFilter * plFilter;

@end

@implementation PeakLimiterFilterVC
...
- (void)audioConfing
{
    ...
    _plFilter = [[AEPeakLimiterFilter alloc] init];
    _plFilter.attackTime = [_oneValueLabel.text floatValue];
    _plFilter.decayTime = [_twoValueLabel.text floatValue];
    _plFilter.preGain = [_threeValueLabel.text floatValue];
}


- (IBAction)onChangeOneValue:(UISlider *)sender {
    _oneValueLabel.text = [NSString stringWithFormat:@"%.3f",sender.value];
    _plFilter.attackTime = [_oneValueLabel.text floatValue];
}

- (IBAction)onChangeTwoValue:(UISlider *)sender {
    _twoValueLabel.text = [NSString stringWithFormat:@"%.3f",sender.value];
    _plFilter.decayTime = [_twoValueLabel.text floatValue];
}
...
- (IBAction)onPlay:(UIButton *)sender {
    ...
    [_audioController addFilter:_plFilter];
...
- (IBAction)onPause:(UIButton *)sender {
    ...
    [_audioController removeFilter:_plFilter];
}
